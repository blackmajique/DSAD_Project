...
class PCA:
    ...
    def getContributiaObservatiilor(self):
        ...
        """
        scoruri = self.scoruri
        eigenvalues_safe = np.where(self.eigenvalues > 0, self.eigenvalues, np.finfo(float).eps)
        # Normalizează scorurile corect, ținând cont de dimensiunile potrivite
        contributii = (scoruri ** 2) / eigenvalues_safe[np.newaxis, :]
        return contributii
